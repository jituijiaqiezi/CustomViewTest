...
class Down360LoadingView extends View {
    ...
    private ValueAnimator loadRotateAnimator;
    //loading下几个运动的点的动画
    private ValueAnimator movePointAnimator;
    //定义4个运动的点
    ...
    //背景收缩的时间
    private int shrinkSpeed = 800;
    //背景收缩后中间的load转一圈需要的时间
    private int shrinkRotateSpeed = 1500;
    //收缩后背景展开的时间
    private int expandSpeed = 1000;
    //loading状态下右边的loading每一次转动时增加的角度
    private int rightLoadingSpeed = 1;
    //左边运动的几个点走一次需要的时间
    ...
    public static class ArgParams {
        ...
        int progressColor;
        int shrinkSpeed;
        int shrinkRotateSpeed;
        int expandSpeed;
        int rightLoadingSpeed;
        int leftLoadingSpeed;
    }

    public void setArgs(ArgParams argParams) {
        statusSize = argParams.statusSize;
        statusColor = argParams.statusColor;
        loadPointColor = argParams.loadPointColor;
        bgColor = argParams.bgColor;
        progressColor = argParams.progressColor;
        shrinkSpeed = argParams.shrinkSpeed;
        shrinkRotateSpeed = argParams.shrinkRotateSpeed;
        expandSpeed = argParams.expandSpeed;
        rightLoadingSpeed = argParams.rightLoadingSpeed;
        leftLoadingSpeed = argParams.leftLoadingSpeed;
        init();
    }
    ...
    private void initAnimation() {
        shrinkAnimation = new Animation() {
            ...
            protected void applyTransformation(float interpolatedTime, Transformation t) {
                ...
                Log.i(TAG,"shrinkAnimation:宽高"+width+"*"+height+",目前长度:"+currentLength+",已运行时间:"+interpolatedTime);
        ...
        shrinkAnimation.setDuration(shrinkSpeed);
        ...
        angleAnimator.setDuration(shrinkRotateSpeed);
        ...
        expandAnimation.setAnimationListener(new Animation.AnimationListener() {
            ...
            public void onAnimationEnd(Animation animation) {
                ...
                status = Status.Load;
                clearAnimation();
                loadRotateAnimator.start();
                movePointAnimator.start();
            }
        ...
        expandAnimation.setDuration(expandSpeed);

        loadRotateAnimator = ValueAnimator.ofFloat(0, 1);
        loadRotateAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                loadAngle += 0.1;
                if (loadAngle > 360) {
        ...
        loadRotateAnimator.setDuration(Integer.MAX_VALUE);

        movePointAnimator = ValueAnimator.ofFloat(0, 1);
        movePointAnimator.setRepeatCount(ValueAnimator.INFINITE);
        movePointAnimator.setInterpolator(new LinearInterpolator());
        movePointAnimator.setDuration(leftLoadingSpeed);
        movePointAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
        ...
        movePointAnimator.addListener(new Animator.AnimatorListener() {
            @Override
            public void onAnimationStart(Animator animation) {
                for (int i = 0; i < fourMovePoint.length; i++) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (status == Status.Normal || status == Status.Start) {
            ...
            float start = (float) (width * 1.0 / 2 - currentLength * 1.0 / 2);
            //Log.i(TAG,"进度长度:"+currentLength+",总长度:"+width);
    ...
    public void setStop(boolean stop) {
        ...
        this.stop = stop;
        if (stop) {
            loadRotateAnimator.cancel();
            moveX = movePointAnimator.getAnimatedFraction();
            movePointAnimator.cancel();
            ...
            loadRotateAnimator.start();
            movePointAnimator.start();
            movePointAnimator.setCurrentPlayTime((long) (moveX * leftLoadingSpeed));
            if (onProgressStateChangeListener != null) {
    ...
    public void setCancel() {
        if (status == Down360LoadingView.Status.Load||status==Status.Complete) {
    ...
    protected void onDetachedFromWindow() {
        ...
        if (loadRotateAnimator != null && loadRotateAnimator.isRunning()) {
            loadRotateAnimator.end();
        }
        if (movePointAnimator != null && movePointAnimator.isRunning()) {
            movePointAnimator.end();
        }
        if (angleAnimator != null && angleAnimator.isRunning()) {
            angleAnimator.end();
        }

        if (shrinkAnimation != null && shrinkAnimation.hasStarted()) {
            shrinkAnimation.cancel();
        }

        if (expandAnimation != null && expandAnimation.hasStarted()) {
